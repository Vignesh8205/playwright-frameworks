...
export class BasePage {
    ...
    constructor(page: Page) {
        ...
        this.title = page.locator('[class="h6 my-3 text-truncate"]')
    }
    ...
    async navigateTo(url: string): Promise<void> {
     ...
     async navigateToPage(pageName:string): Promise<void> {
        ...
        let tabLocator = this.page.locator('[class="h6 my-3 text-truncate"]');
        await this.verifyElementVisible(tabLocator);
    ...
    async verfiyTitle(expectedTitle:string): Promise<void>{
        this.logger.info('Verifying page tile is : ${expectedTitle}');
        await expect(this.title).toHaveText(expectedTitle);
    }
    async clickButtonByRole(buttonName:string): Promise<void>{
        this.logger.info('Clicking on button : ${buttonName}');
        const locator=this.page.getByRole('button', { name: buttonName })
        if(await locator.count() > 1){
            await this.click(locator.first());
        }else{
